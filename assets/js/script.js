...
export class Slideshow {
  constructor(rootEl, opts = {}) {
    ...
    this._loadSlides()
      ...
      .catch((e) => {
        ...
        this.root.innerHTML =
          `<p style="text-align:center; padding:20px; color:#b00;">Failed to load slideshow. ${e.message}</p>`;
      });
  }

  next() { if (this.slides.length) { this._show((this.current + 1) % this.slides.length); this._resetAutoplay(); } }
  prev() { if (this.slides.length) { this._show((this.current - 1 + this.slides.length) % this.slides.length); this._resetAutoplay(); } }
  pause(){ clearInterval(this.timer); this.timer = null; }
  resume(){ if (this.opts.autoplay && !this.isPausedByHoverOrTouch && !this.timer) this._start(); }
  destroy(){ this.pause(); }
  ...
  _prepareDOM() {
    ...
    if (!ar || ar === "auto") { this.stage.style.aspectRatio = "16 / 9"; if (!this.stage.style.minHeight) this.stage.style.minHeight = "320px"; }
    ...
    this.prevBtn = this.root.querySelector('[data-action="prev"]') || this._makeButton("prev", "Previous slide");
    this.nextBtn = this.root.querySelector('[data-action="next"]') || this._makeButton("next", "Next slide");
    if (!this.prevBtn.parentNode){ const w=createEl("div","previous"); w.appendChild(this.prevBtn); this.root.appendChild(w); }
    if (!this.nextBtn.parentNode){ const w=createEl("div","next");     w.appendChild(this.nextBtn); this.root.appendChild(w); }
    ...
    if (this.opts.pauseOnHover) {
      this.root.addEventListener("mouseenter", () => { this.isPausedByHoverOrTouch = true; this.pause(); });
      this.root.addEventListener("mouseleave", () => { this.isPausedByHoverOrTouch = false; this.resume(); });
      this.root.addEventListener("touchstart", () => { this.isPausedByHoverOrTouch = true; this.pause(); }, { passive: true });
      this.root.addEventListener("touchend",   () => { this.isPausedByHoverOrTouch = false; this.resume(); }, { passive: true });
    }
  }
  ...
  _fadeInFirst() {
    ...
    if (first.complete && first.naturalWidth > 0) requestAnimationFrame(reveal);
      ...
      first.addEventListener("error", () => { console.error("Slideshow: failed image", first.src); this._setCaption(0); }, { once: true });
    }
  }
  ...
  _resetAutoplay() { if (this.opts.autoplay) { this.pause(); this.resume(); } }
}
